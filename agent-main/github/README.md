...
## 项目简介
一款基于DeepSeek大模型 + LangChain RAG技术构建的电商智能客服系统，支持用户意图识别、知识库问答、多轮对话等核心功能，适配Windows + Python 3.11环境，无GPU依赖。其中文本嵌入采用本地Ollama服务的bge-m3模型，无需额外API费用。
...
- DeepSeek API密钥（从[platform.deepseek.com](https://platform.deepseek.com)获取）
- Ollama本地服务（用于Embedding模型bge-m3）
...
├── llm_client.py        # DEEPSEEK客户端（HTTP直连）
├── .env                 # 配置文件（需自行创建）
...
### 3. 配置环境变量
创建`.env`文件，填入以下内容（替换为你的DEEPSEEK密钥）：
```ini
# DeepSeek配置
...
OLLAMA_BASE_URL=http://localhost:11434   # 若Ollama不在本机，请修改
...
为了测试系统的召回率以及重排效果，我编写了build_db来完成数据库的搭建
```
...
## 常见问题
### Q1: 报错Ollama服务未启动或connection refused
A1: 请确保Ollama已在本地运行（ollama serve），且.env中的OLLAMA_BASE_URL地址正确（默认为http://localhost:11434）。

### Q2: 提示bge-m3模型未找到
A2: 执行ollama pull bge-m3拉取该模型，确保模型名称与.env中的配置一致即可。

### 客服回复“暂无相关信息”
A3: 未添加知识库文档，调用add_knowledge_doc方法导入相关文档后即可返回精准回答。

## 许可证
本项目仅供学习使用，请勿用于商业用途。

## 免责声明
本项目适配DeepSeek系列模型（deepseek-chat），其他兼容OpenAI接口的模型也可调整后使用

使用前请确保已获取DeepSeek的合法API密钥，并遵守平台使用规范

Ollama为本地服务，请遵守其开源许可条款
